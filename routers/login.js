    ...
    bcrypt.compare(pw, user.user_pw, async (err, same) => {
      ...
      if (same) {
        ...
        const refresh_token = jwt.sign(
          ...
          // process.env.CARD_REFRESH_TOKEN,
          '키키키킼키',
          {
            expiresIn: '1h',
          }
        ...
        console.log(
          `Card 페이지 접속,
          로그인한 유저: ${user.user_id},
          시간: ${time.getMonth() + 1} / ${time.getDate()} - ${(time.getHours() + '').padStart(
            2,
            0
...
router.post('/loginCheck', (req, res) => {
  ...
  jwt.verify(access_token, process.env.CARD_ACCESS_TOKEN, async (acc_err, acc_success) => {
    if (acc_err) {
      jwt.verify(refresh_token, process.env.CARD_REFRESH_TOKEN, async (ref_err, ref_success) => {
        if (ref_err) {
